                        ...
                        <tr data-ticket-id="<?= $ticket['id'] ?>" data-categorie-id="<?= CategorieTicket::getCategorieByName($ticket['categorie'])->getId() ?>">
                            ...
                            <td>
                                <div class="action-buttons">
                                    <button class="btn-affiliate" onclick="affilierTicket(<?= $ticket['id'] ?>)">Détail</button>
                                </div>
                            </td>
                        </tr>
                    <?php endforeach; ?>
...
function filterTickets() {
    ...
    rows.forEach(row => {
        ...
        if (priority && priority !== '' && shouldShow) {
            const priorityCell = row.children[7]; // Colonne priorité
            if (priorityCell) {
        ...
        if (statut && statut !== '' && shouldShow) {
            const statutCell = row.children[5]; // Colonne statut
            if (statutCell) {
        ...
        if ((dateStart || dateEnd) && shouldShow) {
            const dateCell = row.children[4]; // Colonne date
            if (dateCell) {
        ...
        if (searchTerm && shouldShow) {
            const libelleCell = row.children[2]; // Colonne libellé
            const clientCell = row.children[3]; // Colonne client
            
            const libelle = libelleCell ? libelleCell.textContent.toLowerCase() : '';
            const client = clientCell ? clientCell.textContent.toLowerCase() : '';
            
            const searchMatches = libelle.includes(searchTerm) || 
...
function sortTable(columnIndex, ascending = true) {
    ...
    rows.sort((a, b) => {
        ...
        if (columnIndex === 0 || columnIndex === 6) { // ID et Durée
...
function openModal(ticketId) {
    ...
    const currentDuration = row.querySelector('td:nth-child(7)').textContent.trim().split(' ')[0];
